    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 p-6 rounded-xl space-y-6">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <FormInput
            ...
            onChange={(value) => setFormData({ ...formData, fatherHusbandName: value })}
          />
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <FormInput
            ...
            onChange={(value) => setFormData({ ...formData, caste: value.toUpperCase() })}
          />
          
          <FormInput
            label={t('worker.subCaste')}
            value={formData.subCaste || ''}
            onChange={(value) => setFormData({ ...formData, subCaste: value.toUpperCase() })}
          />
        </div>
      </div>
      ...
      <div className="bg-blue-50 p-6 rounded-xl">
        ...
        {formData.dependents?.map((dependent: any, index: number) => (
          <div key={dependent.id} className="bg-white p-4 rounded-lg mb-4 border border-gray-200">
            <div className="flex items-center justify-between mb-4">
            ...
            <div className="grid md:grid-cols-3 gap-4 mb-4">
              <FormInput
                ...
                onChange={(value) => handleDependentChange(dependent.id, 'name', value)}
                required
              />
              
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {t('worker.dependentDob')} <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
              ...
              <FormSelect
                ...
                onChange={(value) => handleDependentChange(dependent.id, 'relationship', value)}
                options={RELATIONSHIPS.map(rel => ({ value: rel.toLowerCase(), label: rel }))}
                required
              />
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <FormSelect
                ...
                onChange={(value) => handleDependentChange(dependent.id, 'isNominee', value)}
                options={[
              ...
              {dependent.isNominee === 'yes' && (
                <FormInput
                  ...
                  onChange={(value) => handleDependentChange(dependent.id, 'benefitPercentage', value)}
                  placeholder="0-100"
                />
              )}
            </div>
